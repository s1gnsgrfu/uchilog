...
export async function POST(request: Request) {
    try {
        ...
        if (commentError || !comment || comment.user_id !== userId) {
        ...
        if (diary.user_id === userId || !diary.is_shared) {
        ...
        const commenterName = profile?.display_name?.trim() || '誰か'
        ...
        const payload = JSON.stringify({
            title: 'UchiLog',
            body: `${commenterName}さんがあなたの日記にコメントしました！`,
            url: `/diary/${diary.id}`,
        })
